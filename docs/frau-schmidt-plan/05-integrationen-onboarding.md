...
- **Out-of-Order-Schutz:** Stripe garantiert keine Zustellreihenfolge. Subscription-Status wird deshalb **nie direkt aus dem Event-Payload** übernommen: Der Webhook-Handler nutzt das Event nur als Trigger und liest den aktuellen Zustand der Subscription per Stripe-API nach (`Subscription.retrieve`), bevor er `subscriptions` aktualisiert. Zusätzlich wird `event.created` je Subscription persistiert und Events, die älter sind als das zuletzt verarbeitete, werden verworfen (Log, kein Fehler).
